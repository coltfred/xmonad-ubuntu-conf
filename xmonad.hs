...
myKeyBindings =
  [
    ...
    , ((myModMask, xK_F11), spawn "changeMobile.sh")
    , ((myModMask, xK_F12), spawn "changeWork.sh")
...
myManagementHooks = [
  ...
  , resource =? "Oracle Beehive Conferencing" --> doFloat
  , resource =? "Notifications - Oracle Beehive Conferencing" --> doFloat
  , className =? "rdesktop" --> doFloat
  , (className =? "Empathy") --> doF (W.shift "1:Chat")
...
myKeys = myKeyBindings ++
  ...
  M.toList (planeKeys myModMask (Lines 2) Finite) ++
  [
